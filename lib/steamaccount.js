...
module.exports = class SteamAccount extends EventEmitter {
  constructor(name, password, games, beOnline, steamladderApiKey) {
    ...
    let refreshToken = fs.readFileSync(
    ...
    this.client.on("steamGuard", async (_, callback) => {
      if (usesTelebot()) {
        ...
        const { code } = await inquirer.prompt([
          { name: "code", message: `Guard code for ${this.name}:` },
        ])
    ...
    this.client.on("loggedOn", () => {
      setLoginQueue((current) =>
        current.filter((acc) => acc.name !== this.name)
      )
      this.loggingIn = false
      this.boost()
    })
  ...
  logheader() {
    return `[${moment().format("YYYY-MM-DD HH:mm:ss")} - ${this.name}]`
  }
  ...
  async safeLogin() {
    if (this.client.loggedOn || this.loggingIn) {
      console.log(
        `${this.logheader()} Previous login attempt still in progress`
      )
    ...
    return new Promise((resolve, reject) => {
      ...
      console.log(
        `${this.logheader()} Unexpected error during login: ${e.message}`
      )
      this.loggingIn = false
      throw e
    })
